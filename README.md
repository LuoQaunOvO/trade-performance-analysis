...
对个人衍生品实盘交易记录(820+完整仓位)进行绩效归因分析的项目,含FXReplay系统化回测复盘。
...
- Bitget U 本位合约导出数据(2025-01 ~ 2026-08):成交明细 / 历史仓位 / 历史委托 / 资金流水
- 成交明细 2394 条,完整仓位 821 个,覆盖 BTC / ETH / SOL / XAU(黄金)等 10+ 品种

## 核心发现

| 指标 | 数值 |
|------|------|
| 仓位盈亏(完整配对口径) | -427.42 USDT |
| 手续费+资金费用 | -204.36 USDT(占亏损约 48%) |
| 仓位胜率 | 47.1%(387盈 / 434亏) |
...
3. **方向性差异显著**:做空仅亏 -29 USDT,做多亏 -398 USDT,逆势做多是主要亏损来源

### 深度分析发现(deep_analysis.py)

| 维度 | 发现 |
|------|------|
| 最大回撤 | -58.75 USDT(月度累计净值) |
| 连续亏损 | 最长连续亏损 17 笔,未失控(风控纪律有效) |
| 单笔期望 | -0.29 USDT(负期望,问题明确) |
| 星期效应 | 周二亏损最多(-159 USDT,206笔),周五盈利(+30) |
...
| 单月平均亏损 | -395 USDT | -50 USDT | 亏损收窄87% |
...
基于 FXReplay 回放工具对价格行为体系进行系统化验证,31 笔完整记录
...
| 胜率 | 58.1%(18胜 / 13负) |
| 总盈亏 | +5.06R(正期望) |
| 平均MFE(R) | 9.14 vs 实际兑现 0.163(兑现率仅1.8%) |

**关键发现**:
1. **形态归因**:双顶(Double Top)+5.96R、楔形(Wedge)+2.13R 为有效形态;L2(-3.02R)、双底(-2.05R)为亏损来源
2. **市场背景**:趋势(17笔,61%胜率)+3.88R、突破模式+2.05R 为正;震荡市 -0.87R 为负
3. **方向归因**:做空 +9.14R vs 做多 -4.08R —— 与实盘 821 仓位结论一致(空优于多)
4. **MFE 兑现率仅 1.8%**:平均理论盈利 9.14R 只兑现 0.16R,与实盘"盈利单拿不住"问题互相印证,
   两份独立数据集指向同一核心缺陷
