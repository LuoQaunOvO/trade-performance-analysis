...
**数据截至 2026-08-17**(数据源:akshare)。

| 指标 | 数值 | 解读 |
|------|------|------|
| 期货主力收盘 | 3016 元/吨 | 价格处于近年低位区间 |
| 最新基差 | -62 元/吨 | 期货升水走扩(现货走弱) |
| 基差率 | -2.01% | 处于近180日36%分位(偏低) |
| **基差率历史分位** | **近180日36%分位** | 两周内从86%分位回落——基差重新走弱,市场状态切换 |
| 库存 | 34417 | 近30日累库 +7895(由去库转为累库) |
| 价格-库存相关性 | r=0.49 | 中等正相关,库存可作价格辅助判断 |
| **盘面利润** | +626 元/吨 | 处于偏高水平,钢厂增产动力强 |

**正套损益测算(含资金成本,年化4.5%)**:当前基差 -62,若收敛至平水 30天净收益约 +51 元/吨;
但累库+基差低分位背景下现货承压,基差大概率继续走弱——正套方向与趋势相悖,风险大于机会。

**跟踪信号表**:以"基差分位 + 库存拐点 + 盘面利润"三维触发(正套观察/收敛止盈/正套风险/减产预期/增产压力),
当前 S3正套风险、S5增产压力已触发。

**研究结论**:两周内框架捕捉到市场状态切换——基差分位 86%→36%、库存去库→累库、相关性 0.59→0.49。
这正是"跟踪框架"的价值:**结论是动态的,框架是常驻的**——当前累库+基差走弱指向现货承压,
等待库存拐点(重新去库)与基差企稳后再评估正套窗口。
